...
export default function PublicAwardPage() {
    const { id } = useParams() as { id: string }
    const [winner, setWinner] = useState<EOTMWinner | null>(null)
    ...
    if (!winner) return <div className="h-screen flex items-center justify-center text-muted-foreground font-bold italic">Award certificate not found.</div>

    return (
        <div className="min-h-screen bg-[#FDFCF9] flex flex-col items-center justify-center p-6 relative overflow-hidden">

            {/* SEO & Meta would be handled in a layout or via Next.js metadata in a real SSR scenario */}
                ...
                <div className="bg-white rounded-[4rem] p-12 shadow-[0_32px_80px_-20px_rgba(0,0,0,0.08)] border-8 border-amber-500/5 relative">
                    ...
                    <div className="flex flex-col items-center text-center">
                        <div className="relative mb-8 sm:mb-10">
                            ...
                            {winner.employee?.profile_image_url ? (
                                <img
                                    ...
                                    className="w-48 h-48 sm:w-64 sm:h-64 rounded-[3rem] sm:rounded-[4rem] object-cover ring-8 ring-white shadow-2xl relative z-10"
                                />
                            ) : (
                                <div className="w-48 h-48 sm:w-64 sm:h-64 bg-amber-500/10 rounded-[3rem] sm:rounded-[4rem] flex items-center justify-center text-amber-500 font-bold text-6xl sm:text-8xl relative z-10 ring-8 ring-white shadow-2xl">
                        ...
                        <div className="space-y-2 mb-8 sm:mb-10">
                            <p className="text-amber-500 font-black uppercase tracking-[0.4em] text-[8px] sm:text-[10px]">Honoring Our Finest</p>
                            <h2 className="text-3xl sm:text-5xl font-black text-slate-900 px-4">{winner.employee?.full_name}</h2>
                            <p className="text-slate-400 font-bold uppercase tracking-widest text-[10px] sm:text-xs pt-4">Employee of the Month — {new Date(winner.reveal_at).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</p>
                        </div>

                        <div className="w-full h-px bg-slate-100 mb-8 sm:mb-10" />

                        {/* Footer Branding */}
                        <div className="flex flex-col sm:flex-row items-center sm:items-end justify-between w-full px-2 sm:px-4 gap-8">
                            <div className="text-center sm:text-left">
                                <p className="text-[10px] uppercase font-black tracking-widest text-slate-400 mb-2 leading-none">Awarded By</p>
                                <div className="flex items-center gap-2 justify-center sm:justify-start">
                                    {org?.logo_url && <img src={org.logo_url} className="h-5 w-5 rounded-md object-contain" />}
                                    <p className="text-xs sm:text-sm font-extrabold text-slate-900 leading-none max-w-[200px] break-words">{org?.name}</p>
                                </div>
                            </div>
                            <div className="text-center sm:text-right border-t sm:border-t-0 pt-6 sm:pt-0 w-full sm:w-auto border-slate-100">
                                <p className="text-[10px] uppercase font-black tracking-widest text-slate-400 mb-2 leading-none">Powered By</p>
